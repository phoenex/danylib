...
class TitanDb
{
	...
	public function __construct()
	{
		...
		{
			// TODO Bunu dusun
			echo "DB error Can not connect to Database with PDO.<br><br>" . $e->getMessage(); exit;
		}
	...
	private function setDatabaseConfig() {
		if(DANY_ENV == "dev") {
			...
			$this->config['db_collation']	= 'utf8_general_ci';
			$this->config['db_prefix']		= '';
			$this->config['db_name']		= "mb";
	...
	public function insert($data = [])
	{
		...
		if (is_null($this->table)) {
			echo 'DB Hatası INSERT işlemi yapılacak tablo seçilmedi.'; exit;
		}
	...
	{
		if (is_null($this->table)) {
			echo 'DB Hatası UPDATE işlemi yapılacak tablo seçilmedi.'; exit;
		}
	...
	{
		if (is_null($this->table)) {
			echo 'DB Hatası DELETE işlemi yapılacak tablo seçilmedi.'; exit;
		}
	...
	{
		if (null !== $this->error) {
			echo 'DB Hatası : ' . $this->error; exit;
		}
	}
